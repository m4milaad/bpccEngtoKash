"""
KATHE 2026 — Reverse Direction Fine-Tuning (Kashmiri → English)
Needed for high-quality back-translation reverse direction.
"""
...
from config import BPCC_DIR, LORA_CONFIG, MODEL_DIR, SRC_LANG, TGT_LANG, TRAIN_CONFIG
...
class DynamicTranslationDataset(Dataset):
    """Tokenized dataset for reverse direction (Kashmiri → English)."""
    ...
    def __getitem__(self, idx):
        ...
        source = str(row["kashmiri"]).strip()
        target = str(row["english"]).strip()
...
def load_training_data():
    """Load BPCC training data for reverse direction."""
    ...
    logger.info(f"[+] Loaded {len(train_df)} training pairs (Kashmiri → English)")
...
def train(args):
    """Main training routine for reverse direction."""
    logger.info("=" * 60)
    logger.info("KATHE 2026 — Reverse Direction Fine-Tuning (Kashmiri → English)")
    logger.info("=" * 60)
    ...
    for epoch in range(args.epochs):
        ...
        if val_loader:
            ...
            if val_loss < best_val_loss:
                ...
                best_dir = MODEL_DIR / "best_reverse"
                best_dir.mkdir(parents=True, exist_ok=True)
                model.save_pretrained(best_dir)
                tokenizer.save_pretrained(best_dir)
                logger.info(f"    [+] New best reverse model saved -> {best_dir}")

        epoch_dir = MODEL_DIR / f"checkpoint-reverse-epoch-{epoch + 1}"
    ...
    logger.info("\n" + "=" * 60)
    logger.info("[+] Reverse Training Complete!")
    logger.info(f"    Best Validation Loss: {best_val_loss:.4f}")
    logger.info(f"    Best Model Saved At:  {MODEL_DIR / 'best_reverse'}")
    logger.info("=" * 60)


def main():
    parser = argparse.ArgumentParser(description="KATHE 2026 Reverse Direction Fine-Tuning")
